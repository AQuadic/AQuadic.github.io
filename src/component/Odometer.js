...
const num_client = String(getUniqueClients(pluck(projects, 'client_id')).length).slice(0,1);
const num_country = String(getUniqueClients(pluck(projects, 'country_id')).length).slice(0,1);
    useEffect(() => {
                        ...
                        <div className="allnum">
                           {num_client.length > 1 ?  num_client :null }
                            <div className="numup" ref={ref2}>
                                <span>  {num_client.length > 1 ?  num_client.slice(1)-1 :num_client-1}  </span>
                                <span> {num_client.length > 1 ?  num_client.slice(1) :num_client} </span>
                            </div>
                    ...
                    <div className="num3 num">
                        <div className="allnum">
                           {num_country.length > 1 ?  num_country :null }
                            ...
                            <span>  {num_country.length > 1 ?  num_country.slice(1)-1 :num_country-1}  </span>
                                <span> {num_country.length > 1 ?  num_country.slice(1) :num_country} </span>
                            </div>
                        </div>
